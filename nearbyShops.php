...
<script>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.petShops.features.forEach(shop => {
                    ...
                    el.textContent = shop.properties.name;
    ...
    function showShopDetails(shop) {
        ...
        const shopDetails = {
            name: shop.properties.name || 'Unknown Name',
            address: shop.properties.full_address || 'Address not available',
            category: shop.properties.feature_type || 'Category not available',
            photo: shop.properties.photo || 'images/default2.png',
            phone: shop.properties.phone || 'Phone not available',
            hours: shop.properties.hours || 'Opening hours not available'
        };

        // Populate sidebar content dynamically
        const sidebarContent = `
        <div class="shop-details-header mb-4">
            <h5 class="fw-bold" style="color:#395065;">
                ${shopDetails.name}
            </h5>
        </div>

        <img src="${shopDetails.photo}" alt="${shopDetails.name}" class="img-fluid mb-3">

        <div class="shop-details-info">
            <p><i class="fa-solid fa-location-dot me-2"></i> ${shopDetails.address}</p>
            <p><i class="fa-solid fa-tag me-2"></i> ${shopDetails.category}</p>
            <p><i class="fa-solid fa-phone me-2"></i> ${shopDetails.phone}</p>
            <p><i class="fa-regular fa-clock me-2"></i> ${shopDetails.hours}</p>
        </div>
    `;
        ...
        const sidebar = new bootstrap.Offcanvas(
            document.getElementById('shopDetailsSidebar')
        );
        sidebar.show();
    }
